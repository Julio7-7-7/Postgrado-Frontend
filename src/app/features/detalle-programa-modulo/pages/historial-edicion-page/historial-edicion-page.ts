...
type EntradaHistorial = HistorialModulo & { diffs: Diff[]; tipos: string[] };

@Component({
  selector: 'app-historial-edicion-page',
  standalone: true,
  imports: [
    CommonModule,
  ...
  templateUrl: './historial-edicion-page.html',
  styleUrl: './historial-edicion-page.css',
})
export class HistorialEdicionPageComponent implements OnInit {
  ...
  modulos = computed(() => {
    ...
    const entradas = this.entradasConDiff();
    for (const entry of entradas) {
      const key = entry.detalle
        ? `${entry.detalle.modulo_sigla} | ${entry.detalle.modulo_nombre}`
        : `Módulo #${entry.id_detalle_programa_modulo}`;
  ...
  private detectarTipos(h: HistorialModulo): string[] {
    const tipos: string[] = [];
    if (h.estado_anterior !== null || h.estado_nuevo !== null) tipos.push('estado');
    if (h.fecha_inicio_original !== null || h.fecha_fin_original !== null) tipos.push('fechas');
    return tipos;
  }

  tipoCambioLabel(tipos: string[]): string {
    if (tipos.includes('estado') && tipos.includes('fechas')) return 'Cambio de estado y fechas';
    if (tipos.includes('estado')) return 'Cambio de estado';
    if (tipos.includes('fechas')) return 'Modificación de fechas';
    return 'Cambio';
  }

  tipoCambioIcon(tipos: string[]): string {
    if (tipos.includes('estado') && tipos.includes('fechas')) return 'swap_horiz';
    if (tipos.includes('estado')) return 'loop';
    if (tipos.includes('fechas')) return 'calendar_month';
    return 'info';
  }

  tipoCambioClass(tipos: string[]): string {
    if (tipos.includes('estado') && tipos.includes('fechas')) return 'tipo-mixto';
    if (tipos.includes('estado')) return 'tipo-estado';
    if (tipos.includes('fechas')) return 'tipo-fechas';
    return 'tipo-default';
  }
  ...
  ngOnInit(): void {
    const match = this.router.url.match(/\/ediciones\/(\d+)\/historial/);
    if (!match) {
  ...
  cargarDatos(): void {
    ...
    this.service.getByEdicion(this.idEdicion()).pipe(
      ...
      error: () => {
        this.error.set('No se pudo cargar el historial de la edición');
        this.isLoading.set(false);
      },
    });
  }
  ...
  volver(): void {
    const base = this.router.url.replace(/\/ediciones\/\d+\/historial.*/, '/ediciones');
